...
static void update_fpsr(CPUM68KState *env, int cc)
{
    ...
    /* Incorporate packed decimal real inexact/operr conversion. */
    if (env->fpsr_pdr) {
        fpsr |= env->fpsr_pdr;
        env->fpsr_pdr = 0;
    }
...
void HELPER(load_pdr_to_fx80)(CPUM68KState *env, FPReg *res, target_ulong addr)
{
     ...
     */
    env->fpsr_pdr = (get_float_exception_flags(&status) & float_flag_inexact
                     ? FPSR_EXC_INEX1 | FPSR_AEXC_INEX : 0);
}
                               ...
                               FPReg *srcp, int kfactor)
{
    ...
    /*
     * Begin with an approximation of log10(x) via the base 2 exponent.
    ...
    res_hi |= (exp10 % 10) << 16;
    res_hi |= ((exp10 / 10) % 10) << 20;
    res_hi |= ((exp10 / 100) % 10) << 24;

    /* If kfactor is too large, or exponent exceeds 3 digits, raise OPERR. */
    if (kfactor > KFACTOR_MAX || exp10 >= 1000) {
        fpsr |= FPSR_EXC_OPERR | FPSR_AEXP_IOP;
        res_hi |= (exp10 / 1000) << 12;
    }
